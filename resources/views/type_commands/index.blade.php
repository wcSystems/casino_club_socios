...
@section('js')
<script>
    $('#type_commands_nav').removeClass("closed").addClass("active").addClass("expand")
    function modal(type,id) {
        ...
        if(id){
            let current={!! $type_commands !!}.find(i=>i.id===id)
    ...
    dataTable("{{route('type_commands.service')}}",[
        {
        ...
        { data: 'name' },
        {
            render: function ( data,type, row  ) {
                return `
                    <a onclick="elim('type_commands',${row.id})" style="color: var(--global-2)" class="btn btn-danger btn-icon btn-circle"><i class="fa fa-times"></i></a>
