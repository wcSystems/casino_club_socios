...
<div class="panel panel-inverse" data-sortable-id="table-basic-1">
    <div class="panel-heading ui-sortable-handle">
        ...
        <div class="panel-heading-btn">
            <button onclick="pdfExport('brand_machines')" class="d-flex btn btn-1 btn-secondary mx-1">
                <i class="m-auto fas fa-lg fa-file-pdf"></i>
            </button>
            <button onclick="excelExport('brand_machines')" class="d-flex btn btn-1 btn-secondary mx-1">
...
@section('js')
<script>
    $('#brand_machines_nav').removeClass("closed").addClass("active").addClass("expand")
    function modal(type,id) {
        ...
        if(id){
            let current={!! $brand_machines !!}.find(i=>i.id===id)
    ...
    function guardar(id) {
        ...
        if(validity){
            ...
            $.ajax({
                url: "{{ route('brand_machines.store') }}",
                type: "POST",
                data: payload,
    ...
    dataTable("{{route('brand_machines.service')}}",[
        {
        ...
        { data: 'name' },
        {
            render: function ( data,type, row  ) {
                return `
                    <a onclick="elim('brand_machines',${row.id})" style="color: var(--global-2)" class="btn btn-danger btn-icon btn-circle"><i class="fa fa-times"></i></a>
