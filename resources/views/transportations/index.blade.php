...
@section('js')
<script>
    $('#transportations_nav').removeClass("closed").addClass("active").addClass("expand")
    function modal(type,id) {
        ...
        if(id){
            let current={!! $transportations !!}.find(i=>i.id===id)
    ...
    function guardar(id) {
        ...
        if(validity){
            ...
            $.ajax({
                url: "{{ route('transportations.store') }}",
                type: "POST",
                data: payload,
    ...
    dataTable("{{route('transportations.service')}}",[
        {
        ...
        { data: 'name' },
        {
            render: function ( data,type, row  ) {
                return `
                    <a onclick="elim('transportations',${row.id})" style="color: var(--global-2)" class="btn btn-danger btn-icon btn-circle"><i class="fa fa-times"></i></a>
