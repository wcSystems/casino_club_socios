...
@section('js')
<script>
    $('#fichas_casinos_nav').removeClass("closed").addClass("active").addClass("expand")
    function modal(type,id) {
        ...
        if(id){
            let current={!! $fichas_casinos !!}.find(i=>i.id===id)
    ...
    function guardar(id) {
        ...
        if(validity){
            ...
            $.ajax({
                url: "{{ route('fichas_casinos.store') }}",
                type: "POST",
                data: payload,
    ...
    dataTable("{{route('fichas_casinos.service')}}",[
        {
        ...
        { data: 'sede_name' },
        {
            render: function ( data,type, row  ) {
                return `
                    <a onclick="elim('fichas_casinos',${row.id})" style="color: var(--global-2)" class="btn btn-danger btn-icon btn-circle"><i class="fa fa-times"></i></a>
