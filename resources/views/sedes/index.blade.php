...
@section('js')
<script>
    $('#sedes_nav').removeClass("closed").addClass("active").addClass("expand")
    function modal(type,id) {
        ...
        if(id){
            let current={!! $sedes !!}.find(i=>i.id===id)
    ...
    dataTable("{{route('sedes.service')}}",[
        ...
        { data: 'name' },
        {
            render: function ( data,type, row  ) {
                return `
                    <a onclick="elim('sedes',${row.id})" style="color: var(--global-2)" class="btn btn-danger btn-icon btn-circle"><i class="fa fa-times"></i></a>
