            ...
            <div data-scrollbar="true" data-height="100%" class="banner-icons">
                    ...
                    @if( Auth::user()->level_id == 4  )
                        ...
                        <li id="cierre_mesas_nav" class="has-sub closed">
                            <a href="{{ route('cierre_mesas') }}">
                                <i class="fas fa-circle text-white"></i>
                                <span class="text-white">CONTEO MAQUINAS</span>
                            </a>
                        </li>
                        <li id="clientes_casinos_nav" class="has-sub closed">
                            <a href="{{ route('clientes_casinos') }}">
...
const clientFilestack = filestack.init("AVqqom3AKTyQqKiPidHE3z");
        ...
        function dataTableClientesCasinos(url,columns,group_name_all, order_by) {
            $(document).ready(function() {
                let table = $('#data-table-default').DataTable({
                    lengthMenu: [[10, 25, 50, 100, -1], [10, 25, 50, 100, "Todos"]],
                    responsive: true,
                    ...
                    drawCallback: function (settings) {
                        ...
                        if(group_name_all){
                            ...
                            api.rows({ page: 'current' }).data().each(function (data, i) {
                                if (last !== data.group_name) {
                                    $(rows).eq(i).before(`<tr class="group_name font-weight-bold"><td colspan="${columns.length}">` + data.group_name + ` 
                                    <a href="${window.location.origin}/fotos-clientes/${data.sede_id}/${data.clasificacion_cliente_casino_id}" target="_blank" class="btn btn-primary m-5"> Visualizar Carpeta </a>
                                    <a href="https://api.whatsapp.com/send?text=${window.location.origin}/fotos-clientes/${data.sede_id}/${data.clasificacion_cliente_casino_id}" class="btn btn-yellow m-5"> Compartir Carpeta </a>
                                     </td></tr>`);
                                    last = data.group_name;
                                }
                            });
