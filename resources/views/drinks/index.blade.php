...
@section('js')
<script>
    $('#drinks_nav').removeClass("closed").addClass("active").addClass("expand")
    function modal(type,id) {
        ...
        if(id){
            let current={!! $drinks !!}.find(i=>i.id===id)
    ...
    dataTable("{{route('drinks.service')}}",[
        {
        ...
        { data: 'name' },
        {
            render: function ( data,type, row  ) {
                return `
                    <a onclick="elim('drinks',${row.id})" style="color: var(--global-2)" class="btn btn-danger btn-icon btn-circle"><i class="fa fa-times"></i></a>
